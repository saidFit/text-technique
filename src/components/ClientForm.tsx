...
const ClientForm: React.FunctionComponent<IShowDataProps> = (props) => {
...
const [address,setAddress] = useState<string>('');
    const handleSumbit = (e:any) =>{
       ...
       if(!name){
        ...
        if(!telephone){
            setErrors((prev:any) => [...prev,'telephone']);
        }else{
            setErrors((prev:any) => prev.filter((item:string,ind:number) => item !== 'telephone'))
           }
        if(!address){
  ...
  return(
    <form onSubmit={(e) => handleSumbit(e)} className='w-full max-w-[400px] my-24 border border-gray-400 pb-8 shadow-MyBox1 rounded-md px-12 mx-auto'>
      ...
      <article className="space-y-3 flex flex-col">
        ...
        <div className="flex flex-col space-y-2">
          <label>
            telephone <span className=" text-red-500 text-xl">*</span>
          </label>
          <input
            className={`${
            errors?.includes("telephone") ? "border border-rose-500" : ""
            } py-3 px-4 border border-gray-300 rounded-md outline-none`}
        ...
        <div className="flex flex-col space-y-2">
          ...
          <input
            className={`${
             errors?.includes("address") ? "border border-rose-500" : ""
            } py-3 px-4 border border-gray-300 rounded-md outline-none`}
